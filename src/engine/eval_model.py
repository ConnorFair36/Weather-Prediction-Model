...
def correlation(ground_truth: np.array, predictions: np.array) -> float:
    ...
    denominator = np.sqrt(np.sum(np.square(ground_truth)) * np.sum(np.square(predictions))) + 1e-9
...
def evaluate_model_by_t(model: nn.Module, dataset: torch.utils.data.dataloader.DataLoader) -> dict:
    """Returns a dictionary containing all of the values used for evaluating model performance based on the given dataset for each timestep."""
    # get the device that the model is currently on
    device = model.parameters().__next__().device
    # loop through the evaluation dataset and save predictions
    predictions = []
    ...
    # regression metrics
    for i in range(3):
        rmse.append(root_mean_squared_error(ground_truth[:,i].flatten(), predictions[:,i].flatten()))
        corr.append(correlation(ground_truth[:,i].flatten(), predictions[:,i].flatten()))
        # meterological metrics
        #   the event being measured is if rainfall >= 0.5 mm is detected
        
        cm.append(confusion_matrix(event_happened[:,i].flatten(), event_predicted[:,i].flatten()))
        csi.append(critical_success_index(cm[-1]))
        far.append(false_alarm_ratio(cm[-1]))
        pod.append(probability_of_detection(cm[-1]))
    return {
        "rmse": rmse,
        "corr": corr,
        "csi": csi,
